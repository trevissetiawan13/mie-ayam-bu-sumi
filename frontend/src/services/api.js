...
export const getSummaryDataByDate = (date) => {
  return axios.get(`/api/transactions/summary/${date}`);
};
...
export const getSummaryRange = (from, to) =>
  apiClient.get(`/transactions/summary-range?from=${from}&to=${to}`);

export const getSummaryData = (period) =>
  apiClient.get(`/transactions/summary?period=${period}`);
