...
router.post("/", (req, res) => {
  ...
  if (!type || !description || amount == null || !date) {
    return res.status(400).json({
      message: "Semua field (type, description, amount, date) diperlukan.",
    });
  }
...
router.get("/summary", (req, res) => {
  ...
  if (period === "daily") {
    // Data 7 hari terakhir, dikelompokkan per hari
    groupByFormat = "%Y-%m-%d";
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(today.getDate() - 7);
    dateCondition = `AND date >= '${sevenDaysAgo.toISOString().split("T")[0]}'`;
  } else if (period === "weekly") {
    // Data 4 minggu terakhir, dikelompokkan per minggu
    ...
    // Data 12 bulan terakhir, dikelompokkan per bulan
    groupByFormat = "%Y-%m";
    ...
    return res.status(400).json({
      message: "Periode tidak valid. Pilih 'daily', 'weekly', atau 'monthly'.",
    });
  }
    ...
    (err, row) => {
      if (err) {
        return res.status(500).json({
          message: "Database error saat memeriksa transaksi",
          error: err.message,
        });
      }
      if (!row) {
      ...
      if (row.user_id !== userId) {
        return res
          .status(403)
          .json({ message: "Anda tidak berhak menghapus transaksi ini." });
      }
      ...
      db.run(sql, [transactionId, userId], function (err) {
        if (err) {
          return res.status(500).json({
            message: "Database error saat menghapus transaksi",
            error: err.message,
          });
        }
        if (this.changes === 0) {
          ...
          return res.status(404).json({
            message:
              "Transaksi tidak ditemukan atau Anda tidak berhak menghapusnya.",
          });
        }
        res
          .status(200)
          .json({ message: "Transaksi berhasil dihapus.", id: transactionId });
      });
    }
  );
});
...
router.get("/summary-range", (req, res) => {
  ...
  if (!from || !to) {
    return res
      .status(400)
      .json({ message: "Tanggal 'from' dan 'to' diperlukan." });
  }
  ...
  db.all(sql, [userId, from, to], (err, rows) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ message: "Gagal mengambil data range." });
    }
    res.json(rows);
  });
});
